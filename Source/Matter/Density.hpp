...
/*!
   ...
   ScalarField()
*/
template <class matter_t> class Density : public Constraints
{
  public:
    ...
    {
        Tensor<2, data_t> h; //!< Conformal metric // ADDED TEMPORARILY BY ROBIN
        data_t chi;          // ADDED TEMPORARILY BY ROBIN
        ...
        void enum_mapping(mapping_function_t mapping_function)
        {
            ...
            using namespace VarsTools; // ADDED TEMPORARILY BY ROBIN
            define_enum_mapping(mapping_function, c_chi,
                                chi); // ADDED TEMPORARILY BY ROBIN
            define_symmetric_enum_mapping(mapping_function,
                                          GRInterval<c_h11, c_h33>(),
                                          h); // ADDED TEMPORARILY BY ROBIN
        }
    };

    //!  Constructor of class Density
    /*!
         Takes in the grid spacing, and matter object plus
         optionally the value of Newton's constant, which is set to one by
       default.
    */
    Density(const matter_t a_matter, double dx, double G_Newton = 1.0);
