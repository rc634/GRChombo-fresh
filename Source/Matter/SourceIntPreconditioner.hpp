...
class SourceIntPreconditioner
{
  public:
    //! Constructor
    SourceIntPreconditioner(const double dx, const double a_L,
                            const std::array<double, CH_SPACEDIM> a_centre,
                            const int a_c_var1 = -1, const int a_c_var2 = -1,
                            const double a_radius = 0.)
        : m_deriv(dx), m_dx(dx), m_L(a_L), m_centre(a_centre),
          m_c_var1(a_c_var1), m_c_var2(a_c_var2), m_radius(a_radius)
    {
    }
    ...
    template <class data_t> void compute(Cell<data_t> current_cell) const
    {
        ...
        data_t original_val1 = current_cell.load_vars(m_c_var1);
        data_t output_val1 = original_val1 * heaviside;
        current_cell.store_vars(output_val1, m_c_var1);

        data_t original_val2 = current_cell.load_vars(m_c_var2);
