...
#if !defined(EMDBHSOLUTION_READ_HPP_)
#error "This file should only be included through EMDBHSolution_read.hpp"
#endif
...
void EMDBHSolution_read::read_from_file()
{
    std::ifstream readfile;

    // maybe guard this ??
    readfile.open("/Users/admin/Documents/robin/GRChombo-fresh/Examples/EMDBH/initial_data/id_lambda-200.dat");
...
void EMDBHSolution_read::insert_strings_to_vecs()
{
...
std::string EMDBHSolution_read::extract_string_from_line(std::string input,
                                                          int imin, int imax)
{
    std::string dummy = "";
...
double EMDBHSolution_read::get_value_interp(const std::vector<double>& in,
...
// 4th order error (cubic interpolation) for field. shouts if asked to fetch a
// value outside the ode solution
double EMDBHSolution_read::get_deriv_interp(const std::vector<double>& in,
                                                          const double r_) const
{
    // index of 2nd (out of 4) gridpoints used for interpolation
    int iter = (int)floor((r_-r[0] )/ dx);
...
#endif /* EMDBHSOLUTION_READ_IMPL_HPP_ */
