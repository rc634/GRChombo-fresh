...
// star tracking now included in BosonStarLevel.hpp in STAMR.hpp
...
void BosonStarLevel::initialData()
{
   ...
   if (m_verbosity)
    ...
    if (m_verbosity)
        pout() << "BosonStarLevel::initialData - Interpolate to 3D grid " << m_level << endl;
    ...
    if (m_verbosity)
        pout() << "BosonStarLevel::initialData - GammaCalc " << m_level << endl;
    ...
    BoxLoops::loop(
        make_compute_pack(
            ...
            MatterConstraints<ComplexScalarFieldWithPotential>(
                ...
                Interval(c_Mom1, c_Mom3)),
            NoetherCharge(),
            EMTensor_and_mom_flux<ComplexScalarFieldWithPotential>(
        ...
        at_level_timestep_multiple(m_p.flux_extraction_level))
    {
        ...
        // if (m_level==m_p.max_level)
        {
            ...
            for (auto level_ptr : all_level_ptrs)
            {
                ...
                BoxLoops::loop(
                    EMTensor_and_mom_flux<ComplexScalarFieldWithPotential>(
                        complex_scalar_field, temp_dx, m_p.L,
